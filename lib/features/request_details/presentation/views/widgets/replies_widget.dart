...
class _RepliesWidgetState extends State<RepliesWidget> {
  ...
  void _fetchIfNeeded() {
    ...
    // if already has data, don't refetch
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<RequestDetailsCubit, RequestDetailsStates>(
      builder: (context, state) {
        ...
        // Show count header + table
        // NOTE: Do NOT wrap in another vertical SingleChildScrollView - RequestDetailsView
        // already has a SingleChildScrollView. Nesting vertical scrollables causes
        // unbounded width constraints -> Row overflow (141px) on small screens.
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              // Optional header with total count
              if (cubit.sentResponsesTotalCount > 0)
                Padding(
                  padding: const EdgeInsets.only(bottom: 8, left: 4),
                  child: Text(
                    'Total replies: ${cubit.sentResponsesTotalCount}  •  Showing ${items.length}',
